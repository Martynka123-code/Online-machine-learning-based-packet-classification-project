...
pcap_compare.py
===============
Standalone PCAP comparison analysis tool with Fixed IP Direction Splitting.
Analyses both FWD and BWD features as separate variables.
...
def calculate_features(packets, label, local_ip):
    """Full feature vector matching RandomForestTrainer's pipeline (STABLE DIRECTION SPLIT)."""
    if not packets:
        return None
    ...
    for p in packets:
        ...
        # Prawidłowy rozdział: pakiety z lokalnego IP komputera -> zawsze FWD, reszta -> BWD
        if src_ip == local_ip:
            fwd.append(p)
        else:
            bwd.append(p)
...
def pcap_to_dataframe(pcap_path: str, label: str, gran: int) -> pd.DataFrame:
    """Read a PCAP file with Auto-IP detection to separate FWD and BWD correctly."""
    flows   = {}
    records = []

    print(f"  ↳ {Path(pcap_path).name}  (class='{label}', gran={gran})")

    # KROK 1: Wykrywanie adresu IP komputera lokalnego
    ip_counts = {}
    ...
    # KROK 2: Właściwa analiza okien pakietowych
    with PcapReader(pcap_path) as reader:
        for pkt in reader:
...
# ═══════════════════════════════════════════════════════════════
# VISUALISATIONS (UNCHANGED ADAPTIVE PLOTS)
# ═══════════════════════════════════════════════════════════════
...
def plot_discrimination_bar(disc_df, out_dir, top_n=20):
    ...
    ax.set_title(f"Top {len(top)} Most Discriminating Features (FWD + BWD split)", fontsize=13, fontweight="bold")
    ax.axvline(0.14, color="#4CAF50", linestyle="--", linewidth=0.8, label="large effect (0.14)")
    ax.legend(fontsize=8)
    ax.grid(axis="x", alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "feature_discrimination_bar.png"), dpi=150, bbox_inches="tight")
    plt.close()
...
def plot_boxplots(df, top_feats, out_dir, top_n=12):
    ...
    for i, feat in enumerate(feats):
        ax = axes[i]
        data_by_class = [df[df["label"] == c][feat].dropna().values for c in sorted(df["label"].unique())]
        labels_list   = sorted(df["label"].unique())
        bp = ax.boxplot(data_by_class, patch_artist=True,
                        medianprops=dict(color="white", linewidth=2),
                        whiskerprops=dict(linewidth=0.8), flierprops=dict(marker=".", markersize=2, alpha=0.4))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "boxplots_top12.png"), dpi=150, bbox_inches="tight")
    plt.close()
...
def plot_violins(df, top_feats, out_dir, top_n=6):
    ...
    for i, feat in enumerate(feats):
        ...
        data_records = []
        for cls in sorted(df["label"].unique()):
            for v in df[df["label"] == cls][feat].dropna().values: data_records.append({"class": cls, feat: v})
        sub = pd.DataFrame(data_records)
        classes  = sorted(sub["class"].unique())
        sns.violinplot(data=sub, x="class", y=feat, order=classes, palette=[cmap[c] for c in classes], inner="box", linewidth=0.8, ax=ax)
    ...
    for j in range(i + 1, len(axes)): axes[j].set_visible(False)
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "violin_top6.png"), dpi=150, bbox_inches="tight")
    plt.close()
...
def plot_heatmap(df, feat_cols, out_dir, top_n=20):
    ...
    for ci in range(len(classes)):
        for fi in range(len(top_feats)):
            ...
            ax.text(fi, ci, txt, ha="center", va="center", fontsize=6.5, color="black" if matrix_norm[ci, fi] < 0.6 else "white")

    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "heatmap_mean_normalized.png"), dpi=150, bbox_inches="tight")
    plt.close()
...
def plot_pairplot(df, top_feats, out_dir, top_n=5):
    ...
    g = sns.pairplot(sub, hue="label", hue_order=classes, palette=dict(zip(classes, [cmap[c] for c in classes])), plot_kws=dict(alpha=0.4, s=10, linewidth=0), diag_kind="kde", corner=True)
    g.figure.savefig(os.path.join(out_dir, "pairplot_top5.png"), dpi=120, bbox_inches="tight")
    plt.close()
...
def derive_label(filename: str) -> str:
    ...
    if "kacper" in filename_lower: return "spotify_kacper"
    elif "agata" in filename_lower: return "spotify_agata"
    elif "martynka" in filename_lower: return "spotify_martynka"
    return Path(filename).stem.split("_")[0].lower()


def main():
    args = parse_args()
    input_dir, output_dir, gran = args.input, args.output, args.gran
    ...
    print(f"\n{'='*60}\n PCAP Comparison Analysis (Full FWD/BWD Split)\n{'='*60}")
    ...
    # POPRAWKA: feat_cols zawiera TERAZ OBIE cechy fwd_ i bwd_ jako osobne kolumny!
    ...
    disc_df.to_csv(os.path.join(output_dir, "feature_discrimination.csv"), index=False)

    top_feats = disc_df["feature"].tolist()
    if top_feats:
        print("\n  Top 10 most discriminating features (Both directions):")
    ...
    if top_feats:
        ...
        if len(top_feats) >= 2: plot_pairplot(df_all, top_feats, output_dir, top_n=min(5, len(top_feats)))
